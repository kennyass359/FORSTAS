...
if(isset($_GET['id']))
{
	...
	$stmt = $pdo->prepare('select c.*, u.login from comments c join users u on c.user_id=u.id where c.post_id = ?');
	$stmt->execute([$_GET['id']]);
					...
					<div id="main">
						...
						<!-- Post -->
							<article class="post">
								...
								<span class="image featured"><img src="<?= $post['img_path']?>" alt="" /></span>
								<p><?= $post['resume']?></p>
								<p><?= $post['message']?></p>
								<footer>
									<ul class="stats">
