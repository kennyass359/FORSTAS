<?php
	...
	$stmt = $pdo->query("select p.*, u.login as login, (select count(*) from comments where post_id=p.id) as count_com, (select count(*) from likes where post_id=p.id) 
	as count_likes from posts p inner join users u on p.user_id=u.id order by timestamp desc limit $art, $kol");
	$posts = $stmt->fetchALL();
?>
<!DOCTYPE HTML>
<!--
					...
					<?php foreach($posts as $key => $post): ?>
							<article class="post">
								...
								<footer>
									...
									<ul class="stats">
										<li><a href="#" class="icon fa-heart liked"><?=$post['count_likes']?></a></li>
